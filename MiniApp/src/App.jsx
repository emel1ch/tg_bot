...
import Adaptation from './pages/Adaptation'           // красивый дизайн для стоматолога
import AdaptationBlood from './pages/AdaptationBlood' // новый компонент для крови
...
export default function App() {
  ...
  const showSuccessToast = () => {
    ...
    setToastPhase('enter')
    toastHideTimerRef.current = window.setTimeout(() => setToastPhase('exit'), 5000)
  ...
  const handleFuturePay = () => ({ ok: true, message: 'Оплата прошла успешно' })
    ...
    <>
      ...
      <style>{`
        @keyframes dentistToastIn {
          from { transform: translate3d(120%, -50%, 0); opacity: 0; }
          to { transform: translate3d(0, -50%, 0); opacity: 1; }
        }
        @keyframes dentistToastOut {
          0% { transform: translate3d(0, -50%, 0); opacity: 1; }
          30% { transform: translate3d(-8px, -50%, 0); opacity: 1; }
          60% { transform: translate3d(6px, -50%, 0); opacity: 1; }
          100% { transform: translate3d(120%, -50%, 0); opacity: 0; }
        }
      `}</style>
    </>
  )
